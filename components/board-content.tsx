...
export const BoardContent = ({
  ...
  const onDragOver = (event: DragOverEvent) => {
    ...
    // 1. Moving a Card over another Card
      ...
      setLists((prev) => {
        ...
        if (activeListIndex === overListIndex) {
          ...
          newLists[activeListIndex] = newActiveList;
           ...
           // --- NEW LOGIC: Check if we are "below" the target to insert after ---
           let newIndex;
    ...
    // 2. Moving a Card over an Empty List
    ...
    if (isActiveACard && isOverAList) {
        setLists((prev) => {
            ...
            if (activeListIndex < 0 || overListIndex < 0) return prev;
            if (activeListIndex === overListIndex) return prev; // If dragging over own list container, ignore
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    if (active.data.current?.type === "Card") {
         ...
         if (activeListIndex >= 0) {
             ...
             const reorderedCards = list.cards.map((card, index) => ({
                 ...card,
                 order: index,
             }));
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={{
          ...
          {activeList && (
             <ListItem index={0} data={activeList} />
          )}
          {activeCard && (
             <CardItem index={0} data={activeCard} />
          )}
      </DragOverlay>
